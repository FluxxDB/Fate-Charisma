local Knit = _G.KnitClient or _G.KnitServer
...
function Zone.new(group, additionalHeight)
	...
	self.playerAdded = maid[maid:GiveTask(Signal.new())]
	self.playerRemoving = maid[maid:GiveTask(Signal.new())]
	self.playerDied = maid[maid:GiveTask(Signal.new())]
	self.updated = maid[maid:GiveTask(Signal.new())]
	...
	for _, part in pairs(self.group:GetDescendants()) do
		if part:isA("BasePart") then
			...
			for _, prop in pairs(partProperties) do
				self._updateConnections:GiveTask(part:GetPropertyChangedSignal(prop):Connect(update))
			end
			for _, event in pairs(groupEvents) do
				self._updateConnections:GiveTask(self.group[event]:Connect(update))
			end
		end
	end
...
function Zone:getPlayer(player)
	...
	if hitValidPart then
		-- Player entered zone
		if not originallyInZone and hum.Health > 0 then
	...
	if originallyInZone and hum.Health == 0 then
		-- Player exited zone
		self.previousPlayers[player] = nil
