...
local function createJointData(attach0, attach1, limits)
	assert(attach0, "[Knit Ragdoll Rigtypes]: Attachment0 must be not nil")
	assert(attach1, "[Knit Ragdoll Rigtypes]: Attachment1 must be not nil")
	assert(limits, "[Knit Ragdoll Rigtypes]: Limits must be provided; got " .. limits)
	assert(limits.UpperAngle >= 0, "[Knit Ragdoll Rigtypes]: Limits.UpperAngle must be higher than -1")
	assert(limits.TwistLowerAngle <= limits.TwistUpperAngle, "[Knit Ragdoll Rigtypes]: Limits.TwistUpperAngle must be higher than Limits.TwistLowerAngle")
...
function RagdollRigtypes.getNoCollisions(model, rigType)
	...
	elseif rigType == Enum.HumanoidRigType.R15 then
		...
		warn("[Knit Ragdoll Rigtypes]: UnknownRigType")
		return {}
	end
end
